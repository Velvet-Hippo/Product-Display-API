...
const getProductStyle = (product_id, cb) => {
  // eslint-disable-next-line quotes
  const queryString = `SELECT styles.style_id, style_name, sale_price, original_price, default_style, jsonb_agg(json_build_object('thumbnail_url', photos.thumbnail_url, 'url', photos.url)) AS photos, json_build_object('quantity', skus.quantity, 'size', skus.size) AS skus FROM styles JOIN photos ON photos.style_id = styles.style_id JOIN skus ON skus.style_id = styles.style_id WHERE styles.product_id = $1 GROUP BY styles.style_id`;
  pool.query(queryString, [product_id], (err, results) => {
    if (err) {
      cb(err, null);
    } else {
...
module.exports = {
  getAllProducts, getProductData, getProductStyle
};
